<template>
  <div class="animated fadeIn">
    <div class="cardheading">
      <h4><i class="fa fa-bars"></i><span class="ml-1">Partner Leave</span></h4>
      <div class="">
      ...
      <div class="">
        <router-link :to="{ name: 'CreatePartnerLeave'}" >
          <button class="btn btn-success">Create New Leave</button>
        </router-link>
      </div>
    </div>
    <b-row>
        ...
        <b-card>
          <v-client-table :data="partners" :columns="columns" :options="options">
            <template slot="action" slot-scope="props">
              <div>
                <router-link :to="{ name: 'EditPartnerLeave', params: { id: props.row.id }}"
                >
                  <span
...
export default {
  ...
  data() {
    return {
      partners : [],
      columns: [
        'id', 'name', 'leave_date', 'type','status','remarks', 'action'
      ],
      partner_id:'',
      inHouse:[],
      options: {
  ...
  created(){
    axios.get(`${ADMIN_URL}/partner-leaves`)
        .then(response => {
  ...
  methods: {
    handleDelete(partner_id) {
      this.$swal({
        ...
        if(result.value) {
          this.deleteRedeem(partner_id);
        }
      }).catch(error => {
        // console.log('Sweetalert ERROR');
      });
    },
    deleteRedeem(partner_id) {

      axios.delete(`${ADMIN_URL}/partner-leave/destroy/${partner_id}`
      )
          .then(response => {
